...
impl<F: TwoAdicField> PeriodicLde<F> {
    ...
    pub fn build(coset: &LiftedCoset, column_evals: &[Vec<F>]) -> Option<Self> {
    ...
    pub fn packed_values_at<P: PackedValue<Value = F>>(
        ...
        (0..self.ldes.width()).map(move |col| {
            P::from_fn(|k| {
                let row = (i + k) % height;
                // SAFETY: `row < height` is guaranteed by the modulo operation,
                // and `col < width` is guaranteed by the iterator bounds (0..self.ldes.width()).
                unsafe { self.ldes.get_unchecked(row, col) }
            })
        })
    }
}
...
mod tests {
    ...
    fn assert_periodic_lde_matches_full(
        ...
        let periodic_lde = PeriodicLde::build(&coset, columns).expect("construction failed");
        ...
        let height = periodic_lde.ldes.height();
        for i in 0..lde_height {
            let row = i % height;
            let actual: Vec<F> = periodic_lde.ldes.row_slice(row).unwrap().to_vec();
        ...
        for start in (0..lde_height).step_by(pack_width) {
            ...
            for k in 0..pack_width {
                let idx = start + k;
                let row = idx % height;
                let scalar: Vec<F> = periodic_lde.ldes.row_slice(row).unwrap().to_vec();
